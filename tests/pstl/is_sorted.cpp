...
void test_is_sorted(Policy&& pol, std::size_t problem_size,
  ...
  auto ret =
      std::is_sorted(pol, data.begin(), data.end());
  auto ret_host =
...
void empty_tests(Policy&& pol) {
  std::cout<< "\n Is_sorted: returns a boolean, 1 (true) if sorted, 0, otherwise.\n";
...
void medium_size_tests(Policy&& pol) {
  test_is_sorted(pol, 20, [](int i){return 20-i;});
  test_is_sorted(pol, 20, [](int i){return (i > 5 ? i : 20-i);});
  test_is_sorted(pol, 20, [](int i){return (i <= 5 ? i : 20-i);});
  test_is_sorted(pol, 20, [](int i){return i;});
  test_is_sorted(pol, 20, [](int i){return 42;});
}
